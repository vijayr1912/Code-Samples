...
def stack_design_matrix(models, X, y_train, cv=LeaveOneOut()):
    p = len(models)
    if type(X) == list: 
        N = X[0].shape[0]
        ...
        X_stack = np.zeros((N,p))
        for i, model in enumerate(models):
            X_stack[:,i] = cross_val_predict(model, X, y_train, cv=cv)
    return X_stack
...
def plot_logistic_regressions(X, y):
    ...
    fig, axes = plt.subplots(row, 3, figsize=(12, rows*(11/4)))
...
def plot_histogram(series):
    ...
    sns.distplot(series, ax=ax, hist_kws={'alpha': 0.9, 'edgecolor':'black'},  kde_kws={'color': 'black', 'alpha': 0.7})
    sns.despine()
    return fig, ax
